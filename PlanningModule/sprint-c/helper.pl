...
schedule_all_surgeries(Room,Day,LOpCode,Tfinal):-
    ...
    availability_all_surgeries(LOpCode,Room,Day,Tfinal),!.
availability_all_surgeries([], _, _, Tfinal) :-
    Tfinal = 0.
availability_all_surgeries([OpCode|LOpCode], Room, Day, Tfinal) :-
    ...
    availability_all_surgeries(LOpCode, Room, Day, Tfinal1),
    Tfinal is max(TfinS, Tfinal1).
